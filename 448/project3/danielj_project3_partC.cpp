...
bool running_time = false;						//flag for printing the running time when a station transmits succes.
float timer = 0;							//current time gets updated after each pass slot*51.2
int array[100];								//dummy array to hold random values 
int array_index=0;							//index of the array
int array_nums;								
int success=0;								//when success = 5 means each station has transmitted once succesfully

int run(int num_of_stations, int rnum, int slot);			//function
class station{
	public:
	...
	void update(){							// when station has sucessfully transmitted reset counter and make nextattempt + 7 slots
		Collision_counter = 0;
		Next_attempt = Next_attempt + 7;
	}
	...
	bool flag_checker( ){
		return transmitted_once;		// function to see if the station has tranmistted atleast once
	} 
	bool update_checker(){
		transmitted_once = true;		// function that updates the flag to signify the station has transmitted once
	}
...
int run(int num_of_stations,int slot){
	...
	if(slot_collision_counter == 1){
		...
		if(S[transmitter_station].flag_checker() == false){
			S[transmitter_station].update_checker();		//station has now transmitted once
			success++;
		}

		running_time = true;				//flag to grab stations running time
		S[transmitter_station].update();		//update call
	}
		...
		for(int i=0; i < num_of_stations; i++){
			if(S[i].transmit_checker(slot)){
				...
				array_index++;
				if(array_index == array_nums)			//loop back to beginning of random number array
					array_index = 0;				
			}
...
int main(int argc, char *argv[] ){
	...
	for(int i=0; i < NUM_STATIONS; i++)
		S[i].reset();			//initalize all statinos
